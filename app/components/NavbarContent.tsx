  ...
  return (
    <nav className="relative flex items-center justify-between p-4 bg-transparent text-gray-900 font-bebas text-2xl lg:text-2xl z-50">
      ...
      <div className="flex items-center space-x-2 z-50">
        ...
        <Link href="/" onClick={closeMenu}>
          <span className="text-2xl font-bold text-green-700 tracking-tight cursor-pointer">CARE4POOR</span>
        </Link>
      </div>
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed top-0 right-0 w-4/5 max-w-xs h-full bg-white text-gray-900 font-bebas text-xl p-6 shadow-lg flex flex-col space-y-6 z-50">
            ...
            <Link href="/">
              <span className="hover:text-green-700 cursor-pointer tracking-tight" onClick={closeMenu}>HOME</span>
            </Link>
            <Link href="/about-us">
              <span className="hover:text-green-700 cursor-pointer tracking-tight" onClick={closeMenu}>ABOUT US</span>
            </Link>
            <button onClick={toggleDropdown} className="hover:text-green-700 cursor-pointer tracking-tight flex items-center">
              PROGRAMS
            ...
            {dropdownOpen && (
              <div className="mt-2 bg-white text-gray-900 p-2 rounded shadow-lg z-20 space-y-2">
                <Link href="/programs/score4change">
                  <span className="block px-4 py-2 hover:text-green-700" onClick={closeMenu}>Score4Change</span>
                </Link>
                <Link href="/programs/debate4change">
                  <span className="block px-4 py-2 hover:text-green-700" onClick={closeMenu}>Debate4Change</span>
                </Link>
              </div>
            )}
            <Link href="/events">
              <span className="hover:text-green-700 cursor-pointer tracking-tight" onClick={closeMenu}>EVENTS</span>
            </Link>
            <Link href="/volunteer">
              <span className="hover:text-green-700 cursor-pointer tracking-tight" onClick={closeMenu}>VOLUNTEER</span>
            </Link>
            <Link href="/contact-us">
              <span className="hover:text-green-700 cursor-pointer tracking-tight" onClick={closeMenu}>CONTACT US</span>
            </Link>
            <Link href="/donate">
              <button className="w-full py-2 bg-green-700 text-slate-100 font-bold rounded-full hover:text-white hover:bg-green-800 transition duration-200" onClick={closeMenu}>
                DONATE
              </button>
            </Link>
          </div>
